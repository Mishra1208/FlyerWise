"""
FlyerWise — Provigo (Quebec) Scraper

Scrapes weekly flyer data from Provigo using the Flipp JSON API.
"""
...
class ProvigoScraper(BaseScraper):
    """Scraper for Provigo weekly flyers using the Flipp API."""

    def __init__(self):
        config = ScraperConfig.STORES["provigo"]
        super().__init__(
    ...
    def extract_products(self) -> list[ScrapedProduct]:
        ...
        logger.info(f"Fetching flyers for Provigo in {postal_code}...")
        data = self.fetch_json(url)
        if not data:
            return []
            
        flyers = data.get("flyers", [])
        provigo_flyers = [f for f in flyers if f.get("merchant") == self.store_name]
        if not provigo_flyers:
            logger.warning(f"No active Provigo flyers found for postal code {postal_code}")
            return []
            
        logger.info(f"Found {len(provigo_flyers)} active Provigo flyer(s)")
        scraped_products = []
        
        for flyer_data in provigo_flyers:
            ...
            logger.info(f"Fetching items for Provigo flyer {flyer_id} ({valid_from} to {valid_to})...")
            ...
            logger.info(f"Retrieved {len(items)} items from Provigo flyer {flyer_id}")
