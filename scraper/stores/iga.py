"""
FlyerWise — IGA (Quebec) Scraper

Scrapes weekly flyer data from IGA using the Flipp JSON API.
"""
...
class IGAScraper(BaseScraper):
    """Scraper for IGA weekly flyers using the Flipp API."""

    def __init__(self):
        config = ScraperConfig.STORES["iga"]
        super().__init__(
    ...
    def extract_products(self) -> list[ScrapedProduct]:
        ...
        logger.info(f"Fetching flyers for IGA in {postal_code}...")
        data = self.fetch_json(url)
        if not data:
            return []
            
        flyers = data.get("flyers", [])
        iga_flyers = [f for f in flyers if f.get("merchant") == self.store_name]
        if not iga_flyers:
            logger.warning(f"No active IGA flyers found for postal code {postal_code}")
            return []
            
        logger.info(f"Found {len(iga_flyers)} active IGA flyer(s)")
        scraped_products = []
        
        for flyer_data in iga_flyers:
            ...
            logger.info(f"Fetching items for IGA flyer {flyer_id} ({valid_from} to {valid_to})...")
            ...
            logger.info(f"Retrieved {len(items)} items from IGA flyer {flyer_id}")
