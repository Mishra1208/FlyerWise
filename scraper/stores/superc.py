"""
FlyerWise — Super C (Quebec) Scraper

Scrapes weekly flyer data from Super C using the Flipp JSON API.
"""
...
class SuperCScraper(BaseScraper):
    """Scraper for Super C weekly flyers using the Flipp API."""

    def __init__(self):
        config = ScraperConfig.STORES["superc"]
        super().__init__(
    ...
    def extract_products(self) -> list[ScrapedProduct]:
        ...
        logger.info(f"Fetching flyers for Super C in {postal_code}...")
        data = self.fetch_json(url)
        if not data:
            return []
            
        flyers = data.get("flyers", [])
        superc_flyers = [f for f in flyers if f.get("merchant") == self.store_name]
        if not superc_flyers:
            logger.warning(f"No active Super C flyers found for postal code {postal_code}")
            return []
            
        logger.info(f"Found {len(superc_flyers)} active Super C flyer(s)")
        scraped_products = []
        
        for flyer_data in superc_flyers:
            ...
            logger.info(f"Fetching items for Super C flyer {flyer_id} ({valid_from} to {valid_to})...")
            ...
            logger.info(f"Retrieved {len(items)} items from Super C flyer {flyer_id}")
